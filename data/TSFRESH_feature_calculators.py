...
@set_property("fctype", "combiner")
def recurrence_analysis5(x, param):
    RECURRENCE_RATE = .05
    ...
    def _extract_line(y, n, linetype):
        if linetype == 'diag':
            return np.diag(y, n)
        elif linetype == 'vert':
            return y[:,n]
    ...
    def _line_hist(y, linetype):
        a_ll = np.array([])
        for n in range(1, N):
            # grab the n-th diagonal
            tmp_line = _extract_line(y, n, linetype)
            # run length encoding
            tmp_rle = _rle(tmp_line)
    ...
    def _entropy(p, be):
        ...
        if n > 0:
            ...
            i = ~ np.isinf(L)
            S = -(P[i] * L[i]).sum()
            a_normprob = np.repeat(1/n, n)
            Snorm = -np.sum(a_normprob*np.log2(a_normprob))
            Sout = S/Snorm 
        else: 
            Sout = np.nan
        return Sout
    ...
    def _lacunarity(y, boxes):
        N = boxes.size
        a_lac = np.zeros(N)
        for n in range(N):
            k = boxes[n]
            nmod = int(y.shape[0]//k*k)
            array = y[:nmod, :nmod]
            tmp_counts = np.add.reduceat(np.add.reduceat(array, np.arange(0, array.shape[0], k), axis=0, dtype=int),
                                   np.arange(0, array.shape[1], k), axis=1, dtype=int)
            mean, var = np.mean(tmp_counts), np.var(tmp_counts)
            # LACUNARITY
            lac = var/(mean**2) + 1
            tmp_compl_counts = k**2 - tmp_counts
            mean, var = np.mean(tmp_compl_counts), np.var(tmp_compl_counts)
            clac = var/(mean**2) + 1
            a_lac[n] = 2 - (1/lac + 1/clac)

        return np.mean(a_lac)
    
    
    def rqa(y, lmin=2):
        # initialize features 
        DET, avgDL, maxDL, Sdiag, LAM, avgVL, maxVL, Svert, avgWL, maxWL, Swhite, tauRR, LAC_short, LAC_long = np.repeat(None, 14)
        
        # diagonal line structures
        a_ll, a_bins, a_nlines  = _line_hist(y, linetype='diag')
        DET = _fraction(bins = a_bins, hist = a_nlines, lmin = lmin)
        Sdiag = _entropy(a_nlines/a_nlines.sum(), a_bins)
        a_llsub = a_ll[a_ll >= lmin]
        if a_llsub.size>0:
            avgDL = np.mean(a_llsub)
            maxDL = np.max(a_llsub).astype(int)
        else: 
            avgDL, maxDL = np.nan, np.nan
        # vertical line structures
        a_ll, a_bins, a_nlines  = _line_hist(y, linetype='vert')
        LAM = _fraction(bins = a_bins, hist = a_nlines, lmin = lmin)
        Svert = _entropy(a_nlines/a_nlines.sum(), a_bins)
        a_llsub = a_ll[a_ll >= lmin]
        if a_llsub.size>0:
            avgVL = np.mean(a_llsub)
            maxVL = np.max(a_llsub).astype(int)
        else: 
            avgVL, maxVL = np.nan, np.nan
        # white vertical line structures/ recurrence times
        RM_inv = 1 - y
        a_ll, a_bins, a_nlines  = _line_hist(RM_inv, linetype='vert')
        Swhite = _entropy(a_nlines/a_nlines.sum(), a_bins)
        a_llsub = a_ll[a_ll >= lmin]
        if a_llsub.size>0:
            avgWL = np.mean(a_llsub)
            maxWL = np.max(a_llsub).astype(int)
        else: 
            avgWL, maxWL = np.nan, np.nan
            
        # lag-1 tau-recurrence rate
        tauRR = np.nanmean(np.diag(y,k=1))
        # lacunarity
        LAC_short = _lacunarity(y, np.arange(5, 25))
        LAC_long = _lacunarity(y, np.arange(int(N/5)-20, int(N/5)))
        
        
        # collect them in dictionary
        d_rqa = dict([('DET', DET), ('avgDL', avgDL), ('maxDL', maxDL), ('Sdiag', Sdiag), 
                      ('LAM', LAM), ('avgVL', avgVL), ('maxVL', maxVL), ('Svert', Svert), 
                      ('avgWVL', avgWL), ('maxWVL', maxWL), ('Swhite', Swhite), 
                      ('tauRR', tauRR), ('LAC_short', LAC_short), ('LAC_long', LAC_long)])     
        return d_rqa
    ...
    ##
    def victhresh(x):
        return np.nanquantile(distmat, RECURRENCE_RATE)
    ...
    calculation = dict(
        ...
        Swhite=get_Swhite,
        tauRR=get_tauRR,
        LAC_short=get_LAC_short,
        LAC_long=get_LAC_long
    )
    
    import scipy
    x = x.reshape((x.size, 1))
    distmat = scipy.spatial.distance.squareform(scipy.spatial.distance.pdist(x, metric='euclidean'), force='tomatrix')
    EPS = victhresh(distmat)
    RM = np.where(distmat<EPS, 1, 0)
    N = RM.shape[0]
    RQA = rqa(RM)

    #fft_abs = np.abs(np.fft.rfft(x))

    res = [calculation[config["rqameasure5"]](RQA) for config in param]
    index = ['rqameasure5_"{}"'.format(config["rqameasure5"]) for config in param]
    return zip(index, res)
...
@set_property("fctype", "combiner")
def recurrence_analysis10(x, param):
    RECURRENCE_RATE = .1
    ...
    res = [calculation[config["rqameasure10"]](RQA) for config in param]
    index = ['rqameasure10_"{}"'.format(config["rqameasure10"]) for config in param]
    return zip(index, res)
...
@set_property("fctype", "combiner")
def recurrence_analysis20(x, param):
    RECURRENCE_RATE = .2
    ...
    res = [calculation[config["rqameasure20"]](RQA) for config in param]
    index = ['rqameasure20_"{}"'.format(config["rqameasure20"]) for config in param]
    return zip(index, res)
...
@set_property("high_comp_cost", True)
@set_property("fctype", "combiner")
def recurrence_network_analysis(x, param):
    
    def rna(y, lmin=2):
        # initialize features 
        ASS, TRANS, DIA = np.repeat(None, 3)
        
        # recurrence network features
        ADJ = np.copy(y)
        ADJ[np.diag_indices_from(ADJ)] = 0
        g_rn = netx.from_numpy_array(ADJ)
        ASS = netx.degree_assortativity_coefficient(g_rn)
        TRANS = netx.transitivity(g_rn)
        DIA = netx.diameter(g_rn.subgraph(max(netx.connected_components(g_rn), key=len)).copy())
    
        d_rna = dict([('assort', ASS), ('transi', TRANS), ('diam', DIA)])        
        return d_rna
    ...
    ##
    def victhresh(x):
        return np.nanquantile(distmat, .1)
    ...
    res = [calculation[config["rnameasure"]](RNA) for config in param]
    index = ['rnameasure_"{}"'.format(config["rnameasure"]) for config in param]
    return zip(index, res)
